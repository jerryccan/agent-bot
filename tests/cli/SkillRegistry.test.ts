...
function fixture(): { root: string; source: string; skillsRoot: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "acp-skill-registry-"));
  directories.push(root);
  const source = path.join(root, "bundled", "acp-bot");
...
describe("SkillRegistry", () => {
  test("defaults to the system-wide agent skills directory", () => {
    ...
    expect(resolveSystemSkillsRoot({ ACP_BOT_SKILLS_DIR: "D:\\shared-skills" }, "ignored"))
  ...
  test("registers, updates, and unregisters its managed skill", () => {
    ...
    expect(fs.readFileSync(path.join(registry.targetPath, "SKILL.md"), "utf8")).toContain("name: acp-bot");
  ...
  test("refuses to replace or remove an unmanaged skill", () => {
    const { source, skillsRoot } = fixture();
    const target = path.join(skillsRoot, "acp-bot");
    ...
    expect(() => registry.install()).toThrow("不由 acp-bot 管理");
