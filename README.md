...
`agent-bot init` prepares the first-run environment. It creates the Agent Bot home, `config.yaml`, `.env`, `data/`, and `logs/` from the bundled examples. When Feishu credentials are missing, it starts Feishu's one-click app registration, displays an authorization URL and QR code, waits for approval, and writes the resulting app ID and secret to `.env`. It then audits the app's published permissions, event subscriptions, and callbacks. If anything is missing, it displays incremental-configuration URLs and QR codes. Initialization waits only for the core message capabilities; missing optional configuration is reported with its affected features and does not block completion. Existing complete credentials are kept unchanged but are still audited.

Use `agent-bot init --skip-feishu` to skip Feishu registration and configuration auditing for console-only setup, or `agent-bot init --reconfigure-feishu` to explicitly replace existing Feishu credentials. The command respects `AGENT_BOT_HOME`, `AGENT_BOT_CONFIG`, and `--config <path>`; add `--json` for structured final output.

`agent-bot server start` launches Agent Bot through a detached resident supervisor. The supervisor automatically restarts Agent Bot after an unexpected exit and applies exponential backoff from 1 to 30 seconds after repeated crashes to avoid a tight crash loop. `npm start` runs the supervisor in the current terminal. For development, use `npm run dev` to run a single process directly.
...
Run `agent-bot init` once after installation. Running it again fills missing paths, preserves existing configuration, and repairs missing published Feishu permissions or subscriptions. Complete Feishu credentials are changed only when `--reconfigure-feishu` is supplied.
...
Initialization normally fills these values in `~/.agent-bot/.env`. To bind an existing enterprise app instead, set both values manually:
...
The one-click registration started by `agent-bot init` uses Feishu's Personal Agent template to create the bot. After creating a new app or loading existing credentials, `init` audits the currently published app version. Missing configuration is split into two independent requests: a core-only incremental link that must be completed first, followed by a non-blocking optional link for any remaining items. This prevents optional administrator approval from holding up the bot's basic message operation.

Core requirements that block initialization:

- Enable bot capability
- Receive events through a persistent connection
- Subscribe to `im.message.receive_v1`
- Have `application:application:self_manage` so `init` can verify the published configuration
- Have the tenant permissions required to receive group @ messages and private messages
- Have `im:message:send_as_bot` or a broader equivalent to send replies

Optional configuration is also requested for richer features:

- `im:chat:create` enables `/newgroup` and `/forkgroup`
- `im:chat:read` with `im.chat.updated_v1` enables group-title synchronization
- `im:message.reactions:write_only` enables message-processing status reactions
- Message-read and resource permissions enable inbound and outbound images and group avatars
- The `card.action.trigger` callback enables interactive card buttons

The configuration link does not contain the app secret. The command lists every missing item before printing the link and polls only while core configuration is missing. If optional items remain unavailable or require separate administrator approval, `init` completes successfully and reports the affected features. Rerun `agent-bot init` later to audit and request them again.
...
- `/model`: show an interactive card containing every supported model, the current model, and reasoning effort. Selecting `Switch` for another model changes the card in place to that model's reasoning-mode selector; `Back` returns from reasoning modes to the model list.
...
- `/status [number-or-Codex-task-ID]`: show the current task, or use a number from the latest `/sessions` result or a task ID to show another task's detailed status, execution steps, and final result. The Status card's `Refresh` action updates the same card in place.
...
- `confirm`: tools that require confirmation display `Allow Once`, `Allow for Session`, `Deny`, and `Cancel Task` actions on the progress card.
