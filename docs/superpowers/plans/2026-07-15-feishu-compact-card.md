...
Stop only the identified acp-bot process tree. Start `D:\dev\acp-bot\.worktrees\codex-app-server\dist\index.js` with working directory `D:\dev\acp-bot`, hidden window, and `ACP_BOT_CONFIG` pointing to the worktree config. Confirm a fresh `Feishu WebSocket connector started` log entry.
