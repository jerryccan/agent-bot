...
`/release` 会停止当前任务所属 Agent 共享的 App Server，以便在 Codex Desktop 中打开这些任务。空闲时先发送释放卡片并倒计时 5 秒；有正在执行或排队的工作时，等待全部结束后释放。**Release Now** 会跳过等待、中断执行并清除排队 Prompt，**Cancel** 会取消本次释放。释放不会归档或删除任务历史，但会影响该 App Server 已加载的所有任务。
